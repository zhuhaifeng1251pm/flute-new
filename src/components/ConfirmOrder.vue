  ...
  methods: {
    ...
    Settlement() {
      ...
      const newMyorder = {
        ...
        coupon:
          this.$store.getters.showCanUseCou.filter(t => t.isselected === true)
            .length * 10,
        goods: this.$store.getters
          .showGoods("isPay")
          .filter(t => t.is_selected === true),
        concession:
          this.$store.getters.showCanUse.filter(t => t.isselected === true)
            .length * 10,
        useIntergral: this.$store.state.personal.personal.useIntegral * 0.01,
