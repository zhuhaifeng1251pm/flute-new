<template>
    <div class="settlement">
        ...
        <div class="main">
            <h3>
                <span>需支付</span>
                <!-- <span>￥{{($store.getters.isToPay-$store.getters.showCanUseCou.filter(t=>t.isselected===true).length*10-$store.getters.showCanUse.filter(t=>t.isselected===true).length*10-$store.state.personal.personal.useIntegral*0.01).toFixed(2)}}</span> -->
                <span>￥{{totals.total-totals.coupon-totals.concession-totals.useIntergral}}</span>
            </h3>
            <div class="pay-way">
                <h3>支付方式：</h3>
        ...
        <Button type='warning' :disabled='($store.getters.isToPay-$store.getters.showCanUseCou.filter(t=>t.isselected===true).length*10-$store.getters.showCanUse.filter(t=>t.isselected===true).length*10)!==0?false:true' @click="toPay">
...
export default {
  ...
  methods: {
    ...
    paysuccess() {
      ...
      this.$store.commit('changeIssend',this.$router.history.current.params.id)
      this.$store.commit("delCoupon");
      this.$store.commit('delconcession')
      this.$router.push("/paysuccess");
      this.$store.commit('lastIntegral')
    }
  }
};
</script>
